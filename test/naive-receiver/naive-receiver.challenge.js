...
describe('[Challenge] Naive receiver', function () {
    ...
    it('Execution', async function () {
        ...
        let attacker = await Attacker.deploy(receiver.address,pool.address);
        if (attacker.deployTransaction.from == deployer.address) {
            console.log("attacker was deployed by deployer");
          } else {
            console.log("attacker was not deployed by deployer");
          }
        //await attacker.addReceiver(receiver.address);
        //await attacker.addPool(pool.address);
        //await attacker.connect(player).att(receiver.address,pool.address);
